...
[`gitcreds_get()`](../reference/gitcreds_get.md) from your package to
query credentials, possibly with a custom URL. For setting new
credentials, you can point your users to
[`gitcreds_set()`](../reference/gitcreds_get.md).
...
- `gitcreds_no_credentials` if git did not find any credentials for the
  specified URL. The URL is stored in the error, under `url`.
...
- `gitcreds_not_interactive_error` if
  [`gitcreds_set()`](../reference/gitcreds_get.md) is called in
...
[`gitcreds_approve()`](../reference/gitcreds-api.md),
[`gitcreds_fill()`](../reference/gitcreds-api.md) and
[`gitcreds_reject()`](../reference/gitcreds-api.md) functions, to
add/update, query and remove credentials. We suggest you use the dummy
credential helper (see below) for
[`gitcreds_fill()`](../reference/gitcreds-api.md), to avoid git password
dialog boxes if a credential is not available.

E.g. the low level API makes it possible to implement an alternative to
[`gitcreds_set()`](../reference/gitcreds_get.md) , with a different user
interface, or a version that also works in non-interactive sessions.
...
[`gitcreds_fill()`](../reference/gitcreds-api.md) adds this dummy helper
to the list of configured credential helpers, and code calling
[`gitcreds_fill()`](../reference/gitcreds-api.md) can check if git
returned the dummy credentials, meaning that no real credentials were
found. This is how the dummy credentials look:
...
If your package uses gitcreds, either directly, or through another
...
cache to the desired value. Use the
[`gitcreds_cache_envvar()`](../reference/gitcreds_cache_envvar.md)
function to see which environment variable you need to set for a url:
...
If you want gitcreds to return a specific credential record, then you
...
If you want gitcreds to fail for a specific host, set the corresponding
environment variable to `"FAIL"`:
...
error class after a `"FAIL:"` prefix, in the environment variable. See
the list of possible error classes above. For example:
...
[`gitcreds_get()`](../reference/gitcreds_get.md) directly and you can
use the mockery package to make gitcreds fail with the desired error
object.
