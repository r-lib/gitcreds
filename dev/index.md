...
Use [`gitcreds_get()`](reference/gitcreds_get.md) to check your GitHub
or other git credentials. It returns a named list, with a `password`
...
Use [`gitcreds_set()`](reference/gitcreds_get.md) to add new
credentials, or replace existing ones. It always asks you before
...
Use [`gitcreds_delete()`](reference/gitcreds_get.md) to delete
credentials. It always asks you before actually deleting any
credentials:
...
[`gitcreds_get()`](reference/gitcreds_get.md). You probably want to
handle the various errors it can return. Here is an example for a
function that optionally neeeds a GitHub token. It searches the code of
a GitHub repository:
...
Point your users to [`gitcreds_set()`](reference/gitcreds_get.md) for
adding/updating their credentials, or write your own wrapper for this.

If you want more control or a different UI, take a look at the lower
level [`gitcreds_fill()`](reference/gitcreds-api.md),
[`gitcreds_approve()`](reference/gitcreds-api.md) and
[`gitcreds_reject()`](reference/gitcreds-api.md) functions.
