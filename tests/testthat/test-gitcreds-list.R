...
gc_test_that(
  ...
  helpers = "osxkeychain",
  {
    ...
    expect_true(
      "github.com" %in% vapply(lst2, function(it) it$attributes$server, "")
    )
    ...
    expect_true(
      "github.com" %in% vapply(lst5, function(it) it$attributes$server, "")
    )
    expect_true(
      "github2.com" %in% vapply(lst5, function(it) it$attributes$server, "")
    )
  }
)
...
gc_test_that("gitcreds_list_manager_core", os = c("windows", "macos"), {
  ...
  mockery::stub(
    gitcreds_list_manager_core,
    "gitcreds_list_manager_core_win",
    "win"
  )
