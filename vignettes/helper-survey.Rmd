...
-   For `git credential approve` git (2.28.0 macOS) does not even call
    the credential helper if no username is supplied:
...
-   The `credential.helper` key has a multi-set value, so if you add a
    new value, the old values are still kept. From git 2.9 specifying an
    empty string removes the previously defined helpers.
...
-   Query without username returns *some* credentials, apparently not
    the ones that were set first:
...
-   No need to supply `username` to get *some* token that matches the
    ...
    Now let's re-add token to make sure that `osxkeychain` does not
    prefer `token`:
    ...
    So it seems that `osxkeychain` will just find an arbitrary one, or
...
-   Install from brew, according to the instructions:
    <https://github.com/microsoft/Git-Credential-Manager-Core#macos-homebrew>
...
-   If updates your global git config, adding these lines:
    ...
    The `helper = ""` line deletes previous handlers. The system helper
    is kept as `osxkeychain`.
...
-   It supports different *providers*. Providers are auto-detected by
    ...
    <https://github.com/microsoft/Git-Credential-Manager-Core/blob/master/docs/configuration.md#credentialprovider>
...
-   In fact the username is completely ignored when getting credentials,
    ...
    <https://github.com/microsoft/Git-Credential-Manager-Core/issues/160>
    `manager` has a similar problem, it is linked from this GitHub
    issue.
...
-   This version adds better support for multiple credentials for the
    same host.
-   It does not save the `username` as part of the service any more, for
    the generic provider.
-   It does save the `username` as `account` and it uses it when
    searching for credentials, both for the `generic` and the `github`
    provider.

## `manager` (GitHub authority)

-   This is Git Credential Manager for Windows, and it is the default
    helper on Windows (for git 2.28.0.windows.1).
...
-   Setting credentials. The `username` must be provided, and it is
    stored in the credential, but the GitHub provider overwrites
    credentials with the same host name, even if they have a different
    `username`. I.e. after this, there will be only one credential:
...
-   It has multiple authorities, (like providers for `manager-core`).
    The GitHub authority is used for `github.com` URLs by default. The
...
-   Supports namespaces, the default namespace is `git`. The namespace
    is included in the name of the credential: `namespace:URL`.

-   There is a pull request to include the `username` in the URL as
    well:
    <https://github.com/microsoft/Git-Credential-Manager-for-Windows/pull/891>

## `manager` (Basic authority)

-   `Basic` authority can be configured with an env var or config
    option:
...
-   But it supports usernames as well. Note that since the username is
    stored in the URL, it does not return some arbitrary credentials if
    we query the URL without a username. We need to tell GCM here not to
    ask for a password, but just fall back to the next credential
    helper.
...
-   Not installed by default (git 2.28.0), but this is supposed to be
...
-   It works similarly to `manager` , I haven't seen any differences in
    behavior (version was 2.0.194.40577). Instead of authorities, we
    need to set up providers:
    <https://github.com/microsoft/Git-Credential-Manager-Core/blob/master/docs/migration.md#gcm_authority.>
    The `Generic` provider works the same way as the `Basic` authority
    in `manager`.

-   Unfortunately setting `GCM_AUTHORITY` will make `manager-core`
    break, so it is not possible to use both `manager` and
    `manager-core` if you need to set this environment variable.

## `manager-core` (on Windows, from 2.0.246-beta

-   This is supposed to support multiple users better, but it does not:
    <https://github.com/microsoft/Git-Credential-Manager-Core/issues/160#issuecomment-700058010>
